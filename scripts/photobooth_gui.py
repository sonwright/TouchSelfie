...
def printPic():
    ...
    #print "command: " + "obexftp --bluetooth " +  printer_MAC +  " --nopath --noconn --uuid none --channel 4 -p " + custom.PROC_FILENAME
    #Button_enabled = False

   	pp = subprocess.Popen(["obexftp --bluetooth " +  printer_MAC +  " --nopath --noconn --uuid none --channel 4 -p " + custom.PROC_FILENAME + " --verbose"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def sendPic(*args):
    if signed_in:
        email = email_addr.get().strip()
        print 'sending photo by email to %s' % email
        if not email:
            print "Email button pressed without email address in textbox"
            can.delete("all")
            can.create_text(WIDTH/2, HEIGHT - STATUS_H_OFFSET, text="No Email was provided.", font=custom.CANVAS_FONT, tags="text")
            can.update()
            time.sleep(1)
            can.delete("all")
            im = Image.open(custom.PROC_FILENAME)
            display_image(im)
            can.create_text(WIDTH/2, HEIGHT - STATUS_H_OFFSET, text="Tap Screen to take another photo", font=custom.CANVAS_FONT, tags="text")
            can.update()
        else:
            try:
